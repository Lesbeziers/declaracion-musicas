...
const I18N = {
  es: {
    ...
    // Modal de aviso al exportar en inglés
    "exportEnModal.title": "⚠ Cue Sheet en inglés",
    "exportEnModal.body": "El Cue Sheet se exportará en <strong>inglés</strong>, válido para productoras extranjeras. <strong>No es válido para la ingesta en NUBE</strong>: si necesitas el fichero para SGAE/NUBE, cambia el idioma a español y vuelve a generarlo.",
    "exportEnModal.continue": "CONTINUAR",
    "exportEnModal.cancel": "CANCELAR",
    "exportEnModal.close": "Cerrar",
    ...
    "info.io.export.body": "Genera el fichero Excel con el formato oficial de SGAE. El idioma del Excel sigue al idioma activo de la app: en <strong>español</strong> es el formato válido para la <strong>ingesta en NUBE</strong>; en <strong>inglés</strong> es solo para productoras extranjeras y <strong>no es válido para NUBE</strong> (se mostrará un aviso al exportar). El botón solo se habilita cuando todos los campos obligatorios están rellenos y los timecodes son correctos. Si hay errores aparece un chip rojo a la izquierda del botón con el número de errores; al pulsarlo, las celdas afectadas se resaltan en el grid.",
  },
  en: {
    "col.titulo": "TITLE",
    "col.autor": "AUTHOR",
    ...
    "exportEnModal.title": "⚠ English Cue Sheet",
    "exportEnModal.body": "The Cue Sheet will be exported in <strong>English</strong>, suitable for foreign producers. <strong>It is not valid for NUBE ingestion</strong>: if you need the file for SGAE/NUBE, switch the app to Spanish and regenerate.",
    "exportEnModal.continue": "CONTINUE",
    "exportEnModal.cancel": "CANCEL",
    "exportEnModal.close": "Close",
    ...
    "info.io.export.body": "Generates the Excel file in SGAE's official format. The Excel language follows the app's active language: in <strong>Spanish</strong> it is the format valid for <strong>NUBE ingestion</strong>; in <strong>English</strong> it is intended only for foreign producers and is <strong>not valid for NUBE</strong> (a warning will be shown on export). The button is only enabled when all required fields are filled in and the timecodes are correct. If there are errors, a red chip appears to the left of the button with the error count; clicking it highlights the affected cells in the grid.",
  },
};
...
function setCellValue(cell, rawValue, historyOptions = {}) {
  ...
  // Resincronizar chip de errores y botón EXPORTAR tras cualquier edición de celda.
  // Sin esto, las ediciones individuales del grid no pasan por renderRows() y dejan
  // chip y botón desactualizados hasta el siguiente disparo (p. ej. tecleo en cabecera).
...
// En inglés sustituimos sharedStrings.xml para que banner, cabeceras de la fila 9
// y los desplegables de la hoja oculta 'Tablas' (que referencian estos shared strings)
// salgan traducidos. Las celdas de modalidad/tipoMusica se escriben luego con tOption()
// para que el valor case con la lista del desplegable.
function translateSharedStringsToEN(xml) {
  const map = new Map([
    ["CUE SHEET DE DECLARACIÓN DE MÚSICAS", "MUSIC DECLARATION CUE SHEET"],
    ["Contacto:", "Contact:"],
    ["Título programa", "Programme title"],
    ["Episodio", "Episode"],
    ["(*) Campo obligatorio", "(*) Required field"],
    ["Intérprete", "Performer"],
    ["Código de librería", "Library code"],
    ["Nombre de librería", "Library name"],
    ["MODALIDAD", "FORMAT"],
    ["TIPO DE MUSICA", "MUSIC TYPE"],
    ["AMBIENTACIONES", I18N.en["mod.AMBIENTACIONES"]],
    ["CARETAS",        I18N.en["mod.CARETAS"]],
    ["FONDOS",         I18N.en["mod.FONDOS"]],
    ["SINFÓNICOS",     I18N.en["mod.SINFÓNICOS"]],
    ["VARIEDADES",     I18N.en["mod.VARIEDADES"]],
    ["LIBRERÍA",       I18N.en["tipo.LIBRERÍA"]],
    ["COMERCIAL",      I18N.en["tipo.COMERCIAL"]],
    ["ORIGINAL",       I18N.en["tipo.ORIGINAL"]],
    ["Título", "Title"],
    ["Autor", "Author"],
    ["Duración\r\n(hh:mm:ss)", "Duration\r\n(hh:mm:ss)"],
    ["Modalidad", "Format"],
    ["Tipo de música", "Music type"],
  ]);
  const decode = (s) => s.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&apos;/g, "'").replace(/&amp;/g, "&");
  const encode = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
  return xml.replace(/<t([^>]*)>([\s\S]*?)<\/t>/g, (full, attrs, body) => {
    const decoded = decode(body);
    const replaced = map.get(decoded);
    if (replaced === undefined || replaced === decoded) return full;
    return `<t${attrs}>${encode(replaced)}</t>`;
  });
}

async function exportCueSheet(options = {}) {
  if (!validateAndExport()) return;

  const isEN = currentLang === "en";
  if (isEN && !options.confirmed) {
    showExportEnWarning();
    return;
  }
  ...
  try {
    ...
    dataRows.forEach((row, i) => {
      ...
      COL_KEYS.forEach((key, j) => {
        ...
        // En inglés los desplegables se escriben con la etiqueta traducida para que
        // case con la lista de la hoja oculta 'Tablas' (también traducida vía sharedStrings).
        const value = isEN && (key === "modalidad" || key === "tipoMusica")
          ? tOption(key, raw)
          : raw;
    ...
    if (isEN) {
      ...
      if (sharedFile) {
        const original = await sharedFile.async("string");
        zip.file("xl/sharedStrings.xml", translateSharedStringsToEN(original));
      }
    }
...
function showExportEnWarning() {
  const overlay = document.getElementById("export-en-overlay");
  if (!overlay) return;
  overlay.classList.add("open");
  overlay.setAttribute("aria-hidden", "false");
  document.body.classList.add("export-en-modal-open");
}

function closeExportEnWarning() {
  const overlay = document.getElementById("export-en-overlay");
  if (!overlay) return;
  overlay.classList.remove("open");
  overlay.setAttribute("aria-hidden", "true");
  document.body.classList.remove("export-en-modal-open");
}
...
function renderApp(root) {
  ...
  root.innerHTML = `
    <section class="panel-layout" aria-label="Declaración de Músicas">
      ...
      <!-- Modal: aviso al exportar en inglés -->
      <div class="draft-import-overlay" id="export-en-overlay" role="dialog" aria-modal="true" aria-labelledby="export-en-title" aria-hidden="true">
        <div class="draft-import-modal">
          <div class="draft-import-modal__header">
            <h2 class="draft-import-modal__title" id="export-en-title" data-i18n="exportEnModal.title">&#x26A0;&nbsp; Cue Sheet en inglés</h2>
            <button type="button" class="draft-import-modal__close export-en-modal__close" data-i18n-aria-label="exportEnModal.close">&#x2715;</button>
          </div>
          <div class="draft-import-modal__body">
            <p data-i18n-html="exportEnModal.body">El Cue Sheet se exportará en <strong>inglés</strong>.</p>
          </div>
          <div class="draft-import-modal__footer">
            <button type="button" class="cue-sheet-btn export-en-modal__cancel" data-i18n="exportEnModal.cancel">CANCELAR</button>
            <button type="button" class="cue-sheet-btn export-en-modal__continue" data-i18n="exportEnModal.continue">CONTINUAR</button>
          </div>
        </div>
      </div>
  ...
  // Modal de borrador importado
  const draftOverlayEl = document.getElementById("draft-import-overlay");
  draftOverlayEl?.querySelector(".draft-import-modal__close")?.addEventListener("click", closeDraftImportModal);
  draftOverlayEl?.querySelector(".draft-import-modal__ok")?.addEventListener("click", closeDraftImportModal);
  draftOverlayEl?.addEventListener("click", (e) => {
    if (e.target === e.currentTarget) closeDraftImportModal();
  });

  // Modal de aviso al exportar en inglés
  const exportEnOverlayEl = document.getElementById("export-en-overlay");
  exportEnOverlayEl?.querySelector(".export-en-modal__close")?.addEventListener("click", closeExportEnWarning);
  exportEnOverlayEl?.querySelector(".export-en-modal__cancel")?.addEventListener("click", closeExportEnWarning);
  exportEnOverlayEl?.querySelector(".export-en-modal__continue")?.addEventListener("click", () => {
    closeExportEnWarning();
    exportCueSheet({ confirmed: true });
  });
  exportEnOverlayEl?.addEventListener("click", (e) => {
    if (e.target === e.currentTarget) closeExportEnWarning();
  });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (document.body.classList.contains("export-en-modal-open")) closeExportEnWarning();
  }, { once: false });
